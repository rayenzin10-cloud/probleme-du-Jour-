...
function highestPopulation(arrayOfObjects) {
  return filter(arrayOfObjects, function (n) {
    return n.population > 500000000;
  });
}
...
function values(obj) {
  var tab1 = [];
  var valeur = Object.values(obj);
  each(valeur, function (n) {
    tab1.push(n);
...
function shortestString(strings) {
  var pluscourt = strings[0];
  each(strings, function (n) {
    if (pluscourt.length > n.length) {
      pluscourt = n;
    }
  });
  return pluscourt;
}
shortestString(["hii", "s", "longest", "hi"])
